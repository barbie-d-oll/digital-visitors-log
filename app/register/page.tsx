...
/**
 * Generates a visitor sign-out code locally.
 *
 * This intentionally does NOT query Firestore.
 * Public visitors should be able to create a visitor record
 * without having read access to the entire visitors collection.
 */
const generateVisitorCode = (name: string) => {
...
export default function PublicVisitorRegistrationPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      /*
       * Generate the visitor code locally.
       *
       * IMPORTANT:
       * We no longer call getDocs() or query() against the
       * visitors collection here.
       */
      const generatedVisitorCode = generateVisitorCode(name);

      let nextSmsStatus: SmsStatus = "idle";

      /*
       * Public visitor registration only needs CREATE permission
       * on the visitors collection.
       */
       ...
       * If SMS fails, the visitor record remains saved.
       */
      try {
        ...
        await updateDoc(visitorRef, {
          ...
          console.error(
            "Error saving SMS failure status:",
            updateError,
          );
        });
      }
      ...
      <div className="relative z-[1] mx-auto grid min-h-svh w-full max-w-[92rem] items-center gap-10 px-[clamp(1.5rem,4.5vw,4.5rem)] pt-28 pb-10 lg:grid-cols-[minmax(18rem,.78fr)_minmax(34rem,1.22fr)] lg:gap-[clamp(3rem,7vw,7rem)] max-[620px]:px-5 max-[620px]:pt-24">
        ...
        <section className="motion-safe:animate-[visitor-reveal_.7s_ease-out_both]">
          ...
          <p className="mt-5 max-w-lg text-[.98rem] leading-7 text-muted-foreground sm:text-[1.05rem]">
            Share a few details and we&rsquo;ll let your host know
            you&rsquo;ve arrived. It only takes a minute.
          </p>
          ...
          <div className="mt-8 flex max-w-md items-start gap-3 rounded-2xl border border-border bg-card p-4 text-muted-foreground shadow-[0_14px_35px_rgba(48,73,68,.07)]">
            ...
            <p className="text-xs leading-5">
              Your details are used only to manage your visit and
          ...
          {isComplete ? (
            ...
            >
              ...
              <p className="mt-3 max-w-sm text-sm leading-6 text-muted-foreground">
                Please have a seat or let the front desk know.
              ...
              <div className="mt-6 w-full max-w-xs rounded-2xl border border-border bg-background p-5 shadow-[0_14px_35px_rgba(48,73,68,.08)]">
                ...
                <p className="mt-3 text-xs leading-5 text-muted-foreground">
                  Keep this code. You&rsquo;ll enter it on the
                  logout page when you are leaving the company.
                </p>
              ...
              {smsStatus === "sent" ? (
                <p className="mt-4 max-w-sm rounded-xl border border-border bg-secondary px-4 py-3 text-xs font-semibold text-secondary-foreground">
                  We also sent this code to your phone number by
                  SMS.
                </p>
              ) : null}
              ...
              {smsStatus === "failed" ? (
                <p className="mt-4 max-w-sm rounded-xl border border-brand-gold/40 bg-accent px-4 py-3 text-xs font-semibold text-accent-foreground">
                  Your check-in was saved, but the SMS could not
                  be sent. Please keep the code shown here.
                </p>
              ) : null}
