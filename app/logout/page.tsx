...
export default function VisitorLogoutPage() {
  ...
  const handleFindVisitor = async (
    ...
    try {
      /*
       * Visitor documents currently have random Firestore IDs.
       *
       * Example:
       *
       * visitors/CFNPQw0DMgBGGzzMb0Kr
       *
       * and inside that document:
       *
       * visitorCode: "BH2999"
       *
       * Therefore, we search by visitorCode instead
       * of assuming the visitor code is the document ID.
       */
      ...
      if (snapshot.empty) {
        setErrorMessage(
          "We couldn't find a visitor with that code. Please check the code or ask the front desk for help.",
        );
        return;
      }

      /*
       * Visitor codes should be unique, so we use the
       * first matching document.
       */
      const visitorDocument = snapshot.docs[0];
      ...
      const visitorRecord: VisitorRecord = {
        ...
        visitorCode:
          getStringValue(data.visitorCode) ||
          normalizedCode,
        checkIn: data.checkIn as FirestoreDate,
      };

      /*
       * Prevent a visitor from signing out twice.
       */
      if (signedOutStatuses.has(visitorRecord.status)) {
        setErrorMessage(
          "This visitor code has already been used to sign out.",
        );
        return;
      }
  ...
  const handleSignOut = async () => {
    ...
    try {
       ...
       * IMPORTANT:
       *
       * visitor.id is the actual Firestore document ID.
       *
       * Example:
       *
       * visitor.id = "CFNPQw0DMgBGGzzMb0Kr"
       *
       * visitor.visitorCode = "BH2999"
       *
       * We update the random Firestore document ID returned
       * from the lookup above.
       */

      await updateDoc(
        doc(db, "visitors", visitor.id),
        {
          status: "Signed Out",
      ...
      <div className="relative z-[1] mx-auto grid min-h-svh w-full max-w-[92rem] items-center gap-10 px-[clamp(1.5rem,4.5vw,4.5rem)] pt-28 pb-10 lg:grid-cols-[minmax(18rem,.82fr)_minmax(32rem,1.18fr)] lg:gap-[clamp(3rem,7vw,7rem)] max-[620px]:px-5 max-[620px]:pt-24">
        <section className="motion-safe:animate-[visitor-reveal_.7s_ease-out_both]">
          ...
          <p className="mt-5 max-w-lg text-[.98rem] leading-7 text-muted-foreground sm:text-[1.05rem]">
            Enter the sign-out code you received after
            check-in. We&rsquo;ll show your visitor details for
            confirmation, then record your leaving time.
          </p>
          ...
          <div className="mt-8 flex max-w-md items-start gap-3 rounded-2xl border border-border bg-card p-4 text-muted-foreground shadow-[0_14px_35px_rgba(48,73,68,.07)]">
            ...
            <p className="text-xs leading-5">
              Your checkout time is captured only after you
              confirm the visitor details in the pop-up.
            </p>
          </div>
        </section>
        ...
        <section className="rounded-[1.6rem] border border-border bg-card/95 p-5 shadow-[0_30px_80px_rgba(48,73,68,.13)] backdrop-blur-3xl motion-safe:animate-[visitor-panel_.75s_.1s_cubic-bezier(.22,1,.36,1)_both] sm:p-8 lg:p-10">
          <div className="mb-7">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Enter the visitor code you received after
              registration.
            </p>
          </div>
          ...
          <div className="p-7 max-[520px]:p-5">
            <DialogHeader className="pr-9 text-left">
              ...
              <DialogDescription className="text-[.86rem] leading-6 text-muted-foreground">
                Confirm before signing out so the leaving time
                is recorded on the correct visitor record.
              </DialogDescription>
            </DialogHeader>
